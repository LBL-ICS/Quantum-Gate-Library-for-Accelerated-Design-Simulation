...
object FPUnits {
  ...
  class FP_sqrt(bw: Int, L: Int) extends Module {
    ...
    val check_if_0 = !io.in_a(bw-2, 0).orR //Added in this project to get rid of 0 -> some decimal

    io.out_valid := fsqrt.out_valid
    io.out_s := Mux( check_if_0, 0.B, out_sign ## out_exp(exponent - 1, 0) ## out_frac)
  }
  ...
  class FP_mult(bw: Int, pd: Int) extends Module {
    ...
    val check_if_0 = !io.in_a(bw-2, 0).orR || !io.in_b(bw-2, 0).orR //Added in this project to get rid of 0 -> some decimal
    val plus1      = io.in_a === io.in_b || out_frac.andR //Squaring the sqrt(2) is 1 digit away from being correct

    io.out_s      := Mux(check_if_0, 0.U,ShiftRegister(out_sign ## out_exp ## out_frac,sr_array(9),io.in_en) + plus1)
  ...
  class FP_add(bw: Int, pd: Int) extends Module {
    ...
    io.out_valid  := ShiftRegister(io.in_valid, pd, io.in_en)
    io.out_s      := Mux(check_if_0, 0.U,norm_out_sign ## norm_out_exp ## norm_out_frac)
  }
