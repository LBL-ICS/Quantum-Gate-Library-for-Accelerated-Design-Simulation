...
*/
class TopQSU(num_of_qubits : Int, bit_width : Int, mult_pd : Int, add_pd : Int) extends Module{
  val io = IO(new Bundle{
    ...
    val in_applyGate        = Input(Bool()) //Applies the input gate onto the register
  ...
  val gatePool    = Module(new QGP(num_of_qubits, bit_width, mult_pd, add_pd))
